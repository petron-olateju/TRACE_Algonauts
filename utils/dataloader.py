...
def get_fmri_file_path(dir: str, subject: int, split: str = "train") -> str:
    """Build the file path for an fMRI data file."""
    loader = get_default_dataset()
    return loader.get_fmri_file_path(subject, split)


def list_fmri_sessions(dir: str, subject: int, split: str = "train") -> List[str]:
    """List available fMRI session keys in an HDF5 file."""
    loader = get_default_dataset()
    return loader.list_fmri_sessions(subject, split)
...
def load_fmri_responses(
    ...
    """Load fMRI response data for a specific stimuli session."""
    loader = get_default_dataset()
    return loader.load_fmri_responses(stimuli_name, split)
...
def load_transcript(
    ...
    """Load transcript for a given stimuli session."""
    loader = get_default_dataset()
    return loader.load_transcript(stimuli_name, split, ignore_nans)
...
def epoch_fmri_by_words(
    ...
    hrf_delay: int = None,
    tr: float = None,
    context_trs: int = None,
    split: str = "train",
) -> Tuple[Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray]]:
    """Create epoched fMRI response windows aligned to word onsets."""
    loader = get_default_dataset()
    return loader.epoch_fmri_by_words(
        stimuli_name, fmri_data, transcript_dir, fmri_dir,
        hrf_delay, tr, context_trs, split
    )


def load_episode_fmri(subject, split, fmri_dir):
    """Load continuous fMRI timeseries for all episodes."""
    loader = get_default_dataset()
    return loader.load_episode_fmri(subject, split, fmri_dir)


def load_episode_word_epochs(subject, split, fmri_dir, transcript_dir,
                              hrf_delay, tr, context_trs):
    """Load word-locked fMRI epochs for all episodes."""
    loader = get_default_dataset()
    return loader.load_episode_word_epochs(subject, split, fmri_dir, transcript_dir)


def load_atlas_for_subject(
    subject: int, dir: str = None
) -> Tuple[np.ndarray, np.ndarray, Dict[int, Dict[str, Any]]]:
    """Load brain atlas data for a specific subject."""
    loader = get_default_dataset()
    return loader.load_atlas_for_subject(subject)


def get_parcel_label(coord: np.ndarray) -> Optional[str]:
    """Get the Schaefer atlas label for a 3D coordinate."""
    loader = get_default_dataset()
    return loader.get_parcel_label(coord)